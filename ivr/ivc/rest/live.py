...
def includeme(config):
    ...
    config.add_route('live_stream', '/{tenant}/cameras/{camera_id}/live')
    config.add_route('live_stream_keepalive', '/{tenant}/cameras/{camera_id}/live/keepalive')
...
def request_live_stream(request):
    ...
    url, session_id = request.registry.stream_mgr.request_live_stream(request.matchdict['tenant'],
...
def delete_live_stream(request):
    ...
    request.registry.stream_mgr.delete_live_stream(request.matchdict['tenant'],
                                                   request.matchdict['camera_id'],
...
def keepalive_live_stream(request):
    ...
    request.registry.stream_mgr.keepalive_live_stream(request.matchdict['tenant'],
                                                      request.matchdict['camera_id'],
                                                      req['session_id'])
