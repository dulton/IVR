...
class IVT(RPCSession):
    ...
    def get_camera(self, vendor, camera_id):
    ...
    def rtmp_publish_stream(self, vendor, camera_id, publish_url):
        self._send_request('RTMPPublish', {'camera_id': camera_id,
                                           'url': publish_url,})

    def stop_rtmp_publish(self, vendor, camera_id, publish_url):
...
class CameraManager(object):
    ...
    def _find_ivt(self, vendor, camera_id):
        for ivt in self._ivts.itervalues():
            if camera_id in ivt:
                return ivt

    def iter_camera(self, vendor):
        for ivt in self._ivts.itervalues():
            for camera in ivt.iter_camera(vendor):
                yield camera

    def get_camera(self, vendor, camera_id):
        ivt = self._find_ivt(vendor, camera_id)
        if not ivt:
    ...
    def camera_cnt(self, vendor):
        total_camera = 0
        for ivt in self._ivts.itervalues():
            total_camera += ivt.camera_cnt(vendor)
        return total_camera

    def rtmp_publish_stream(self, vendor, camera_id, publish_url):
        ivt = self._find_ivt(vendor, camera_id)
        if ivt:
            ivt.rtmp_publish_stream(vendor, camera_id, publish_url)
        else:
            raise IVRError("{0} camera {1} does not exit".format(vendor, camera_id))

    def rtmp_stop_publish(self, vendor, camera_id, publish_url):
        ivt = self._find_ivt(vendor, camera_id)
        if ivt:
            ivt.stop_rtmp_publish(vendor, camera_id, publish_url)
        else:
            raise IVRError("'{0}' camera '{1}' does not exist".format(vendor, camera_id))
...
class StreamManager(object):
    ...
    def get_live_streams(self, vendor, camera_id):
        if camera_id not in self._streams:
            raise IVRError('No live stream found for camera {0}'.format(camera_id))
        return self._streams[camera_id]

    def request_live_stream(self, vendor, camera_id, stream_format='hls', stream_quality='low', keepalive_required=False, create=True):
        for _ in xrange(5):
            ...
            if camera_id in self._streams and stream_format in self._streams[camera_id]:
                ...
                if create:
                    stream = self._create_stream(vendor, camera_id, stream_format, keepalive_required)
                    return stream['url']
                else:
    ...
    def delete_live_stream(self, vendor, camera_id, session_id=None, force=False):
        if camera_id not in self._streams:
        ...
        if force:
            ...
            self._destroy_stream(vendor, camera_id, stream_format)
        else:
    ...
    def keepalive_live_stream(self, vendor, camera_id, session_id):
        if camera_id not in self._streams:
            raise IVRError("No stream {0} from camera {1}".format(stream_format, camera_id))
        if stream_format not in self._streams[camera_id]:
            raise IVRError("No stream {0} from camera {1}".format(stream_format, camera_id))
    ...
    def _create_stream(self, vendor, camera_id, stream_format, keepalive_required):
        ...
        if stream_format == 'hls':
            ...
            self._camera_mgr.rtmp_publish_stream(vendor, camera_id, rtmp_publish_url)
            stream['ready'] = True
            return stream
        else:
            raise IVRError('Unsupported stream format {0}'.format(stream_format))

    def _destroy_stream(self, vendor, camera_id, stream_format):
